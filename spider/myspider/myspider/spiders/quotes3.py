# -*- coding: utf-8 -*-
import scrapy
from ..items import MyspiderItem_specialityList

class Quotes3Spider(scrapy.Spider):
    name = 'quotes3'
    allowed_domains = ['zuihaodaxue.com']
    start_urls = ['http://www.zuihaodaxue.com/BCSR/zhexue2017.html']
    def parse(self, response):
        item = MyspiderItem_specialityList() 
        title = response.xpath('/html/body/div[3]/div/div[2]/div/div[1]/span/text()').extract()
        sp_name = title[0].replace('软科中国最好学科排名','').replace('2018-','').replace('2017-','').replace('2019-','')
        item['sp_name'] = sp_name
        sp_year = title[0].replace('软科中国最好学科排名','').replace('-','').replace(sp_name,'')
        item['sp_year'] = int(sp_year)
        quotes = response.xpath('//div[@class="news-text"]/table/tbody/tr[@class="bgfd"]')
        print ("-------------------start--------------------")
        for quote in quotes:
            # item = MyspiderItem_specialityList() 
            item['ranking'] = int(quote.xpath('td[1]/text()').extract()[0].replace(' ','').replace('\r','').replace('\n',''))
            # item['sp_dep'] = quote.xpath('td[2]/div/text()').extract()[0].replace(' ','').replace('\r','').replace('\n','')
            # item['sp_name'] = quote.xpath('td[2]/div/text()').extract()[0].replace(' ','').replace('\r','').replace('\n','')
            if sp_year == '2017':
                #爬取2017年的
                item['u_name'] = quote.xpath('td[3]/text()').extract()[0].replace(' ','').replace('\r','').replace('\n','')
            else:
                #爬取2018/2019
                item['u_name'] = quote.xpath('td[4]/text()').extract()[0].replace(' ','').replace('\r','').replace('\n','')
        ...
        spList = ['哲学','经济学','法学','教育学','文学','历史学','理学','工学','农学','医学','管理学','艺术学']
